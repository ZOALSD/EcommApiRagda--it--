...
<head>
    ...
    <link href="{{ url($path) }}/design/admin_panel/assets/global/plugins/simple-line-icons/simple-line-icons.min.css"
        rel="stylesheet" type="text/css" />
    <link
        href="{{ url($path) }}/design/admin_panel/assets/global/plugins/bootstrap/css/bootstrap{{ app('direction') }}.min.css"
        rel="stylesheet" type="text/css" />
    <link
    ...
    <link href="{{ url($path) }}/design/admin_panel/assets/pages/css/login{{ app('direction') }}.min.css"
        rel="stylesheet" type="text/css" />
...
<body class=" login">
    ...
    <div class="logo">
        <a href="{{ aurl('login') }}">
            @if (!empty(setting()->logo))
                <img src="{{ it()->url(setting()->logo) }}" alt="{{ setting()->{l('sitename')} }}"
        ...
        <form class="login-form" method="post">
            ...
            <div class="form-actions">
                ...
                <a href="javascript:;" id="forget-password"
                    class="forget-password">{{ trans('admin.forgot_password') }}</a>
            </div>
        </form>
        <!-- END LOGIN FORM -->
    ...
    <div class="copyright">
        ...
        @foreach (L::all() as $lang)
            <a style="color:{{ app('l') == $lang ? '#c33' : '#fff' }}"
                href="{{ aurl('lang/' . $lang) }}">{{ trans('admin.' . $lang) }}</a>
            <?php $i++; ?>
            {{ $i < count(L::all()) ? '.' : '' }}
        @endforeach
        <br />
        <bdi> 2014 © Metronic. Admin Dashboard Template & <a href="http://phpanonymous.com/it" target="_blank">Deployed
                By It Version 1.0</a> </bdi>
    </div>
    <!--[if lt IE 9]>
    ...
    <script src="{{ url($path) }}/design/admin_panel/assets/global/plugins/bootstrap/js/bootstrap.min.js"
        type="text/javascript"></script>
    <script src="{{ url($path) }}/design/admin_panel/assets/global/plugins/js.cookie.min.js" type="text/javascript">
    </script>
    <script src="{{ url($path) }}/design/admin_panel/assets/global/plugins/jquery-slimscroll/jquery.slimscroll.min.js"
        type="text/javascript"></script>
    <script src="{{ url($path) }}/design/admin_panel/assets/global/plugins/jquery.blockui.min.js"
        type="text/javascript"></script>
    <script src="{{ url($path) }}/design/admin_panel/assets/global/plugins/bootstrap-switch/js/bootstrap-switch.min.js"
        type="text/javascript"></script>
    <!-- END CORE PLUGINS -->
    <!-- BEGIN PAGE LEVEL PLUGINS -->
    <script src="{{ url($path) }}/design/admin_panel/assets/global/plugins/jquery-validation/js/jquery.validate.min.js"
        type="text/javascript"></script>
    <script
        src="{{ url($path) }}/design/admin_panel/assets/global/plugins/jquery-validation/js/additional-methods.min.js"
        type="text/javascript"></script>
