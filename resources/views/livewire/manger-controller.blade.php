                ...
                <tr>
                    ...
                    <td>
                        @foreach ($i->roles as $r)
                            <button wire:click="adminRoleSelect({{ $r->id }}, '{{ $i->email }}')"
                                class="btn btn-success" data-toggle="modal" data-target="#Change_Role">
                                {{ $r->name }}</button>
                        @endforeach
                    </td>
                    <td>
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <i class="fa fa-exclamation-triangle"></i> هـل انـت متأكــيد مــن حـــذفـ المـشرف ؟ <i
                                    class="wirring">{{ $i->name }}</i>
                            </div>
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <input wire:model.lazy="name" type="text" class="form-control" placeholder="إسم المشرف"
                                    name="name" id="">
                                @error('name') <div class="error">{{ $message }}</div> @enderror

                                <br>
                                <input wire:model.lazy="email" type="email" class="form-control"
                                    placeholder="البريد الالكتروني" name="email" id="">
                ...
                <div wire:ignore.self class="modal fade" id="Change_Role">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <button class="close" data-dismiss="modal">x</button>
                                <h4 class="modal-title"> تغير دور المشرف {{ $admin_name }} من {{ $role_selected_name }}
                                </h4>
                            </div>
                            <div class="modal-body">
                                ...
                                <select wire:model.lazy='admin_Role_up' name="admin_Role_up" id="" class="form-control">
                                    @foreach (Spatie\Permission\Models\Role::pluck('name') as $role)
                                        <option @if ($role_selected_name == $role)
                                            selected
                                    @endif
                                    value="{{ $role }}">
